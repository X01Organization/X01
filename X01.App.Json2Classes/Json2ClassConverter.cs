...
namespace Json2Class
{
    public class Json2ClassConverter
    {
        ...
        {
            DirectoryInfo outputDirectoryInfo = new DirectoryInfo(_option.OutputDirectory);
...
#region TEST
            ...
            Dictionary<string, ClassInfo> classInfoDictionary = new();
            try
            {
                await foreach (string json in GetJsonsAsync(true, token))
                {
                    ...
                    sw.Flush();
                    ms.Position = 0;
                    using JsonDocument jsonDocument = await JsonDocument.ParseAsync(ms, cancellationToken: token);
            ...
            foreach (ClassInfo x in classInfoDictionary.Values)
            {
            ...
            [EnumeratorCancellation] CancellationToken token)
        {
            ...
            {
                string cacheFilename = $"c:/workroot/tmp/{tripId}.json";
                ...
                else
                {
                    ...
                    HttpResponseMessage response = await client.GetAsync($"https://www.journaway.com/aggregator/valhalla/gettrip?tripId={tripId}");
            ...
            CancellationToken token)
        {
            ...
            if (!classInfoDictionary.TryGetValue(className, out ClassInfo? classInfo))
            {
                classInfo = CreateClassInfo(className);
                classInfoDictionary.Add(className, classInfo);
            }

            foreach (JsonProperty x in jsonElement.EnumerateObject())
            {
                token.ThrowIfCancellationRequested();
                PropertyInfo1 propertyInfo = new PropertyInfo1()
                {
                ...
                if (JsonValueKind.Array == x.Value.ValueKind)
                {
                    ...
                    {
                        JsonValueKind elementsValueKind = x.Value.EnumerateArray()
                        ...
                        {
                            string propertyClassName = className + propertyInfo.Name;
            ...
            CancellationToken token)
        {
            ...
            JsonElement[] elements = jsonElement.EnumerateArray().ToArray();
            JsonValueKind elementsValueKind = elements.Select(x => x.ValueKind).Distinct().Single();
            ...
            foreach (JsonElement x in elements)
            {
                token.ThrowIfCancellationRequested();
        ...
        private async Task WriteClassAsync(ClassInfo classInfo, CancellationToken token)
        {
            ...
            PropertyInfo1[] invalidProperties = classInfo.Properties
            ...
            string classFileName = Path.Combine(_option.OutputDirectory, classInfo.Name + ".cs");
            await using FileStream fs = File.OpenWrite(classFileName);
            await using StreamWriter sw = new StreamWriter(fs);
            foreach (string? x in classInfo.Usings.Distinct().OrderBy(x => x))
            {
                token.ThrowIfCancellationRequested();
            ...
            PropertyInfo1[] allProperties = classInfo.Properties
                                         .DistinctBy(x => $"{x.Name}<>{x.JsonName}<>{x.TypeName}")
                                         .ToArray();

            foreach (PropertyInfo1? x in allProperties.Where(x => "null" == x.TypeName))
            {
                string[] typeNames = allProperties.Where(y => y.Name == x.Name &&
                                                         "null" != y.TypeName)
                                             .Select(x => x.TypeName)
                                             .Distinct()
                                             .ToArray();
            ...
            foreach (PropertyInfo1? x in allProperties.Where(x => "[]" == x.TypeName))
            {
            ...
            IGrouping<string, PropertyInfo1>[] notUniqueNames = allProperties.GroupBy(x => x.Name).Where(x => x.Skip(1).Any()).ToArray();
            if (0 < notUniqueNames.Length)
            {
                //throw new InvalidOperationException("Name must be unique");
            }

            IGrouping<string, PropertyInfo1>[] notUniqueJsonNames = allProperties.GroupBy(x => x.JsonName).Where(x => x.Skip(1).Any()).ToArray();
            if (0 < notUniqueJsonNames.Length)
            {
                //throw new InvalidOperationException("JsonName must be unique");
            }

            foreach (PropertyInfo1? x in allProperties.OrderBy(x => x.JsonName))
            {
                token.ThrowIfCancellationRequested();
