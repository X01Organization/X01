namespace ComputopParameterParser.Util
{
    public class StringUtil
    {
        ...
        {
            string[] sa = s.Split(dimiliter);
            List<string> sl = sa.Take(sa.Length - 1).Select(x => x + dimiliter).ToList();
            sl.Add(sa.Last());
        ...
        {
            char[] dilimiters = new[] { '.', '!', '?', ',' };
            string[] sa = SplitByDimiliter(s, '.').Select(x => SplitByDimiliter(x, '!').Select(x => SplitByDimiliter(x, '?')
